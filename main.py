...
class DatabaseManager:
    ...
    def get_user(self, user_id):
        ...
        if self.db_url:
            try:
                res = requests.get(f"{self.db_url}/users/{uid}.json", timeout=6)
    ...
    def save_user(self, user_id, data):
        ...
        if self.db_url:
            try:
                requests.put(f"{self.db_url}/users/{uid}.json", json=data, timeout=6)
                return
            except Exception as e:
    ...
    def has_number_received_otp(self, number):
        ...
        if self.db_url:
            try:
                res = requests.get(f"{self.db_url}/processed_numbers/{num}.json", timeout=6)
                if res.status_code == 200 and res.json() is not None:
    ...
    def mark_number_received_otp(self, number):
        ...
        if self.db_url:
            try:
                requests.put(f"{self.db_url}/processed_numbers/{num}.json", json=True, timeout=6)
                return
            except Exception as e:
    ...
    def save_withdraw(self, req_id, data):
        ...
        if self.db_url:
            try:
                requests.put(f"{self.db_url}/withdraws/{r_id}.json", json=data, timeout=6)
                return
            except Exception as e:
    ...
    def get_withdraw(self, req_id):
        ...
        if self.db_url:
            try:
                res = requests.get(f"{self.db_url}/withdraws/{r_id}.json", timeout=6)
                if res.status_code == 200 and res.json() is not None:
                    return res.json()
            except Exception as e:
    ...
    if os.path.exists(CONFIG_FILE):
        try:
            with open(CONFIG_FILE, "r") as f:
                ...
                loaded["OTP_DESTINATIONS"] = ["-1004309875319"]
                return loaded
        except:
...
def get_country_info_by_range(range_val):
    ...
    prefix_map = {
        ...
        "232": "Sierra Leone 🇸🇱",
        "228964": "Togo (Moov) 🇹🇬",
        "23672": "Central African Republic 🇨🇫",
...
# --- 100% BULLETPROOF BROADCAST ENGINE (ALL USERS) ---
def process_broadcast(message):
    chat_id = message.chat.id
    msg_id = message.message_id
    
    all_target_users = list(set(all_users).union(db.get_all_user_ids()))
    target_uids = [int(uid) for uid in all_target_users if int(uid) > 0 and int(uid) != int(config["ADMIN_ID"])]
    
    if not target_uids:
        safe_send_message(chat_id, "❌ **ব্রডকাস্ট ব্যর্থ!**\n\nডাটাবেজে কোনো ইউজার পাওয়া যায়নি।")
        return

    total_users = len(target_uids)
    status_msg = safe_send_message(chat_id, f"🚀 **ব্রডকাস্ট প্রক্রিয়া শুরু হচ্ছে...**\n\n• মোট ইউজার: `{total_users}` জন\n• অনুগ্রহ করে অপেক্ষা করুন...")
    ...
    def broadcast_worker():
        ...
        for index, uid in enumerate(target_uids, 1):
            ...
            while not sent and retries < 2:
                try:
                    bot.copy_message(chat_id=uid, from_chat_id=chat_id, message_id=msg_id)
                    success += 1
                    sent = True
                except telebot.apihelper.ApiTelegramException as e:
                    if e.error_code == 429: # Rate limit reached
                        retry_after = 5
                        try:
            ...
            # প্রোগ্রেস আপডেট প্রতি ৫০ জন বা লাস্ট ইউজারে
            if index % 50 == 0 or index == total_users:
                ...
                try:
                    bot.edit_message_text(
                        ...
                        text=f"📊 **ব্রডকাস্ট অগ্রগতি:** `{index}/{total_users}`\n\n"
                             f"✅ সফল: `{success}`\n"
        ...
        safe_send_message(chat_id, f"🎉 **ব্রডকাস্ট সম্পন্ন!**\n\n"
                                   ...
                                   f"• ব্যর্থ/ব্লকড: `{failed}` জন\n"
                                   f"• মোট সময়: `{elapsed_total}` সেকেন্ড")
...
def request_number(call):
    bot.answer_callback_query(call.id, text="⚡ Provisioning Number...")
    _, country, selected_app = call.data.split("_")
    rid = config["SERVICES"][selected_app]["rids"].get(country)
    
    if not rid:
        safe_send_message(call.message.chat.id, "⚠️ এই দেশের কোনো সচল রেঞ্জ পাওয়া যায়নি!")
        return
    ...
    try:
        response = requests.post(url, json=payload, headers=get_api_headers(), timeout=15)
        res = response.json()
        meta = res.get("meta", {})
        ...
        if meta.get("code") == 200 or meta.get("status") == "success":
            ...
            safe_send_message(call.message.chat.id, f"❌ স্টক খালি: {res.get('message', 'অন্য একটি দেশ বেছে নিন')}")
    except Exception as e:
        safe_send_message(call.message.chat.id, "⚠️ কানেকশন সমস্যা! আবার চেষ্টা করুন।")
...
def manual_fetch(call):
    ...
    if not found:
        bot.answer_callback_query(call.id, text="⚠️ ওটিপি এখনো আসেনি! অটোমেটিক ট্রাই করা হচ্ছে...", show_alert=True)
...
def check_and_send_otp_manual(chat_id, selected_app, country, num, message_id=None):
    ...
    try:
        res = requests.get(url, headers=get_api_headers(), timeout=12).json()
        meta = res.get("meta", {})
        if meta.get("code") == 200 or meta.get("status") == "success":
            ...
            if found_msg:
                ...
                # ১. ইউজারের DM এ সম্পূর্ণ তথ্য ও ফোন নম্বর যাবে
                ...
                # ২. একই ওটিপি নোটিফিকেশন গ্রুপেও পোস্ট হবে
                group_text = (f"🔥 **USER OTP RECEIVED** 🔥\n\n"
                              f"📱 Service: {icon} {selected_app.upper()}\n"
                              f"📞 Number: `{format_group_phone_number(num)}`\n"
                              f"🔑 OTP Code: `{isolated_code}`")
                for dest in config.get("OTP_DESTINATIONS", ["-1004309875319"]):
...
# --- REAL-TIME LIVE CONSOLE SMS MONITOR ENGINE (GROUP DESTINATION ONLY) ---
def background_live_sms_monitor():
    global seen_console_hits, range_hits_tracker, last_announced_range
    while True:
        try:
            time.sleep(2.5) # ২.৫ সেকেন্ড পর পর দ্রুত স্ক্যান
            base_url = str(config['BASE_URL']).strip().rstrip('/')
            url = f"{base_url}/numsuccess/info"
            ...
            if meta.get("code") == 200 or meta.get("status") == "success":
                ...
                for item in otps_list:
                    ...
                    # শুধুমাত্র নির্দিষ্ট ওটিপি গ্রুপে রিয়েলটাইম ওটিপি স্ট্রিম পোস্ট হবে
                    for dest_id in config.get("OTP_DESTINATIONS", ["-1004309875319"]):
                        try:
                            ...
                            print(f"Error posting live stream: {ex_post}")
        except Exception as e:
            time.sleep(3)
...
def sync_services_once():
    ...
    try:
        ...
        response = requests.get(url, headers=get_api_headers(), timeout=12)
...
def check(call):
    if is_subscribed_all(call.from_user.id): 
        ...
        send_home_keyboard(call.message.chat.id, "✅ ভেরিফিকেশন সফল! এখন থেকে সার্ভিস ব্যবহার করতে পারবেন।")
    else: 
